...
#define LPOCEAN_VERTEX_INCLUDED
    ...
    #define LPOCEAN_WAVE_HORIZONTAL_OFFSET(pass, index, colorFrom) uv = GetWorldWaveUV(worldPos.xz, _OceanWaveRadian.##pass, _OceanWaveRect##index);\
        uv.y -= _OceanWaveSpeedZ.##pass * _LPOceanTime;\
        tempValue = colorFrom;\
        worldPos.xz += float2(_OceanWaveHeightPow.##pass, _OceanWaveHeightScale.##pass) * tempValue;

    #define LPOCEAN_WAVE_HEIGHT(pass, index, colorFrom) uv = GetWorldWaveUV(worldPos.xz, _OceanWaveRadian.##pass, _OceanWaveRect##index);\
        uv.y -= _OceanWaveSpeedZ.##pass * _LPOceanTime;\
        tempValue = colorFrom;\
