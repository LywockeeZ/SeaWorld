...
namespace JiongXiaGu.LowpolyOcean
{
    ...
    public static class VertexHelper
    {
        ...
        public static float GetWaveHeight(this OceanShaderOptions ocean, Vector3 worldPos, float time)
        {
            Texture2D waveTexture = ocean.Wave.Texture as Texture2D;
            if (waveTexture == null)
                return 0;

            float height = 0;

            switch (ocean.Mode.Wave)
            {
                case WaveMode.Wave1:
                    height += GetWaveHeight(ocean.Wave, waveTexture, worldPos, 0, 1, 2, time);
                    break;

                case WaveMode.Wave2:
                    height += GetWaveHeight(ocean.Wave, waveTexture, worldPos, 0, 1, 0, time);
                    height += GetWaveHeight(ocean.Wave, waveTexture, worldPos, 2, 3, 1, time);
                    break;

                case WaveMode.Wave3:
                    height += GetWaveHeight(ocean.Wave, waveTexture, worldPos, 0, 0, time);
                    height += GetWaveHeight(ocean.Wave, waveTexture, worldPos, 1, 1, time);
                    height += GetWaveHeight(ocean.Wave, waveTexture, worldPos, 2, 2, time);
                    break;
            }
        ...
        public static float GetWaveHeight(this OceanShaderOptions ocean, Vector3 worldPos)
        {
            return GetWaveHeight(ocean, worldPos, DefaultTime);
        }
        ...
        public static float GetRippleHeight(this OceanShaderOptions ocean, Vector3 worldPos)
        {
            if ((ocean.Mode.Ripple & RippleMode.Max4) != 0)
            {
        ...
        public static float GetVertexHeight(this OceanShaderOptions ocean, Vector3 worldPos, float time)
        {
            float height = GetWaveHeight(ocean, worldPos, time);
            height += GetRippleHeight(ocean, worldPos);
            return height;
        }

        public static float GetVertexHeight(this OceanShaderOptions ocean, Vector3 worldPos)
        {
            return GetVertexHeight(ocean, worldPos, DefaultTime);
        }
    }
}
