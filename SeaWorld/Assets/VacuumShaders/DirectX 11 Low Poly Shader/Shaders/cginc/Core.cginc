...
inline void ApplyDissolve(float alpha, inout float3 albedo)
{
	#if defined(V_LP_DISSOLVE) && defined(V_LP_CUTOUT) 

		if ((_Cutoff * _DissolveEdgeSize) > 0 && _DissolveEdgeSize > alpha)
		{
			float edgeGradient = 1 - saturate(alpha) * (1.0 / _DissolveEdgeSize);
			edgeGradient *= edgeGradient * edgeGradient;

			float lerpStrength = lerp(1, edgeGradient, _DissolveEdgeType);

			albedo = lerp(albedo, lerp(albedo, _DissolveEdgeColor.rgb, _DissolveEdgeColor.a), lerpStrength);
		}
...
#define SETUP_LP_PIXEL_UV(o,v) o.pixelTexUV.xy = v.texcoord.xy;
#else
