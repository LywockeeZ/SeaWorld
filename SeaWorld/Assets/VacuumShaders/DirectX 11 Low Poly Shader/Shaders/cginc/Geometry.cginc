...
void geom(triangle v2f_surf input[3], inout TriangleStream<v2f_surf> triStream)
{
	...
	//UV
	#ifdef V_GEOMETRY_SAVE_LOWPOLY_UV
		float2 lowPolyUV = lerp((input[0].pixelTexUV.xy + input[1].pixelTexUV.xy + input[2].pixelTexUV.xy) / 3.0, input[0].pixelTexUV.xy, _SamplingType);
		
		input[0].pixelTexUV.zw = lowPolyUV;
		input[1].pixelTexUV.zw = lowPolyUV;
		input[2].pixelTexUV.zw = lowPolyUV;
	#endif	 
